...
- 私聊直接发送任务
- 群聊通过 `@机器人` 发送任务
- 同一聊天自动复用 CLI session
- 群聊通过 `/bind <工作目录> [仓库名]` 绑定固定目录
- 空目录绑定时会在线拉取 `vibe-coding-standard` 模板，再初始化 Git / GitHub 仓库
- 非空目录绑定时不会覆盖现有内容，只继续准备 Git / GitHub 仓库
...
- 如果目录是空的，`/bind` 会先在线拉取 `vibe-coding-standard` 模板结构
- 如果目录已有内容，`/bind` 不会覆盖原文件

### 常用命令

- `/help`：查看帮助
- `/bind <工作目录> [仓库名]`：绑定当前群组目录；空目录会先拉模板，再准备 Git / GitHub 仓库
- `/status`：查看当前聊天状态
